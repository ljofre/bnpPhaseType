...
#'fys = mcmcErlangMix( Y=y, a=0.1, b=0.1, aa=2, bb=0.1,
#'                    alpha=1, beta=1, nscan=10000, nburn=2000, nskip=8 )
...
mcmcErlangMix <-
           ...
           nskip)
  {
    ...
    for (k in 2:nscan)
    {
      ...
      # testear que el resultado este bueno # pasar a funciones vectoriales
      ...
      W <- rbind.fill.matrix(W,matrix(w, nrow = 1))
      temp3 = rgamma(n = 1,
                     shape = aa,
                     rate = bb)
      R = c(R, temp3)
      ...
      if (k >= nburn)
      {
        ...
        if (clave == 0)
        {
          ...
          for (j in 1:N)
          {
            sumtemp1 = sumtemp1 + w[j] * dgamma(x = tau.grid,
                                                shape = KR[j],
                                                rate = lambda)
          }
          fys[cont, ] = sumtemp1
      ...
      if (sum((k == indexprint) * 1) == 1)
      {
        print(paste0(
          "running iterations ",
          indexprint[max((k == indexprint) * jj)] - 1,
          " to ",
          max((k == indexprint) * jj) * 1000,
          " of ",
          nscan
        ))
      }
